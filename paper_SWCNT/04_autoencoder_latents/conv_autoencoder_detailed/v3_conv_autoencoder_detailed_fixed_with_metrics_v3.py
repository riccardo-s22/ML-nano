...
def train_epoch(model, dataloader, optimizer, device, alpha=1.0, beta=1.0):
    ...
    Returns:
        avg_loss, avg_recon_loss, avg_class_loss, accuracy
    """
    model.train()
    ...
    for images, labels in dataloader:
        ...
        reconstructions, class_logits, _ = model(images)
    ...
    avg_recon_loss = total_recon_loss / max(len(dataloader), 1)
    avg_class_loss = total_class_loss / max(len(dataloader), 1)
...
def validate(model, dataloader, device, alpha=1.0, beta=1.0):
    ...
    Returns:
        avg_loss, avg_recon_loss, avg_class_loss, accuracy, attention_weights (numpy array)
    """
    model.eval()
    ...
    attention_arr = np.concatenate(all_attention_weights, axis=0) if all_attention_weights else np.empty((0, 0))
    return avg_loss, avg_recon_loss, avg_class_loss, accuracy, attention_arr

def compute_fold_metrics(model, dataloader, device, positive_label: int = 1):
    """Compute confusion matrix + metrics for a trained model on a dataloader."""
    model.eval()
    y_true = []
    y_pred = []
    y_prob = []
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            ...
            _, class_logits, _ = model(images)
            probs = torch.softmax(class_logits, dim=1)
            preds = torch.argmax(probs, dim=1)

            y_true.extend(labels.detach().cpu().numpy().tolist())
            y_pred.extend(preds.detach().cpu().numpy().tolist())
            y_prob.extend(probs[:, positive_label].detach().cpu().numpy().tolist())
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(codes, labels)):
        ...
        # ========== EVALUATE BEST MODEL (CONFUSION MATRIX + METRICS) ==========
        best_model_path = f'best_model_fold{fold+1}.pth'
        if os.path.exists(best_model_path):
            model.load_state_dict(torch.load(best_model_path, map_location=device))
        else:
            # Fallback: if (rarely) no best model file was written, evaluate current weights
            torch.save(model.state_dict(), best_model_path)

        best_metrics = compute_fold_metrics(model, val_loader, device, positive_label=1)

        print(f"\nBest Model Metrics (Fold {fold+1}) on Validation Subjects:")
        print(f"  Confusion matrix [[TN, FP], [FN, TP]]: {best_metrics['confusion_matrix'].tolist()}")
        ...
        if np.isnan(best_metrics['auc']):
            print(f"  AUC          : NaN (only one class present in this validation fold)")
        else:
        ...
        # Attach best-model metrics for this fold (computed on validation set)
    ...
    # ========== METRICS SUMMARY (BEST MODEL PER FOLD) ==========
    metric_keys = ['accuracy', 'sensitivity', 'specificity', 'precision', 'recall', 'f1', 'auc']
    metrics_by_fold = {k: [] for k in metric_keys}

    for r in fold_results:
        bm = r.get('best_metrics', {})
        for k in metric_keys:
            metrics_by_fold[k].append(bm.get(k, float('nan')))
    ...
    def _nanstd(x):
        arr = np.array(x, dtype=float)
        return float(np.nanstd(arr)) if not np.all(np.isnan(arr)) else float('nan')

    print(f"\nOverall Classification Metrics (Best Model Per Fold):")
    for k in metric_keys:
        mean_v = _nanmean(metrics_by_fold[k])
        std_v = _nanstd(metrics_by_fold[k])

        if k == 'auc':
            if np.isnan(mean_v):
                print(f"  AUC: NaN (AUC undefined in one or more folds due to single-class validation)")
            else:
                print(f"  AUC: {mean_v:.3f} ± {std_v:.3f}")
        else:
            if np.isnan(mean_v):
                print(f"  {k}: NaN")
            else:
                print(f"  {k}: {mean_v*100:.2f}% ± {std_v*100:.2f}%")
