...
def validate(model, dataloader, device, alpha=1.0, beta=1.0):
    ...
    avg_class_loss : float
        Average classification (cross-entropy) loss.
    accuracy : float
        Classification accuracy (percent) over the validation set.
    attention_weights_all : np.ndarray
    ...
    avg_recon_loss = total_recon_loss / len(dataloader) if len(dataloader) > 0 else float("nan")
    avg_class_loss = total_class_loss / len(dataloader) if len(dataloader) > 0 else float("nan")
    accuracy = 100.0 * correct / total if total > 0 else 0.0
