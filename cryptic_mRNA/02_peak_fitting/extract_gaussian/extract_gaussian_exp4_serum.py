...
Output chirality_gaussian_descriptors_exp4_serum.csv."""
...
BASE="/mnt/c/Users/riccardo-s/Documents/CNT/cryptic_mRNA/exp4/serum"; PREFIX="STMN_22_Serum__"; NCOL=12; MISS=90
...
for w in range(nwell):
    ...
    meta=dict(plate="exp4_serum",matrix="serum",hours=2,w=w,gp=p,row=r,col=c,well=f"r{r}c{c}",cond=f"C{c:02d}")
    for ch,cc in CHIRALITY.items():
        d=fit_chirality(em, spec[ex_map[ch]][:,w], cc["em"]); d["ex_used"]=ex_map[ch]
        for k in DESC: meta[f"{ch}_{k}"]=d[k]
    rows.append(meta)
df=pd.DataFrame(rows); out=f"{BASE}/chirality_gaussian_descriptors_exp4_serum.csv"; df.to_csv(out,index=False)
