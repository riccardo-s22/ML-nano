#!/usr/bin/env python
# force_correction_v5_take_closest_or_autopeaks.py
#
# Fix for force_correction_v4.py:
# - Coordinates_DNA.txt is treated as COORDINATES ONLY; any "height" columns/values are ignored.
# - "Take closest coordinate":
#     * If coords file contains nm (Emission/Excitation or Em/Ex): snap each point to the closest pixel on the EEM grid.
#     * If coords file contains ONLY chirality labels (e.g., 6,5 7,6 ...): automatically pick the closest REAL
#       spectral coordinates by auto-detecting the strongest peaks from the PBS mean EEM.
# - Surface is interpolated on the BATCH-2 grid and applied with strict axis/shape checks.
# - Writes debug files for QC.
#
# Typical usage:
# python force_correction_v5_take_closest_or_autopeaks.py ^
...
#   --output_dir "...exp7\\0h_corrected_force_v5" ^
#   --recursive --debug
#
import argparse
import os
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Force batch correction surface (XLSX/CSV). Uses coords if nm are present; "
                    "otherwise auto-detects peak anchors from PBS mean (chirality-only coords files)."
    )

    p.add_argument("--batch1_pbs_folder", required=True, help="Folder containing Batch 1 PBS controls")
    p.add_argument("--batch2_pbs_folder", required=True, help="Folder containing Batch 2 PBS controls")
...
def load_eem(file_path: str, sheet_name=None, sheet_index=0):
    """
    Loads EEM from CSV or XLSX.
    Returns: em_axis(sorted), ex_axis(sorted), data(sorted), df_sorted, em_col, ex_cols_sorted
    """
...
def parse_nm_coords_or_chiralities(coords_file: str):
    """
    Returns:
      nm_points: list[(Ex, Em)] parsed from keywords (Emission/Excitation or Em/Ex). Heights ignored.
      chiralities: list[(n, m)] parsed from patterns like '8,3' (used only to infer count)
    """
...
def snap_points_to_grid(points_ex_em, ex_axis, em_axis):
    """
    For each (Ex,Em) point, snap to the closest EEM pixel coordinate on the grid.
    Returns array shape (k,2): (Ex_snapped, Em_snapped)
    """
    snapped = []
...
def sample_matrix_at_points(em_axis, ex_axis, matrix, points_ex_em):
    ...
    vals = np.asarray(interp(pts_em_ex), dtype=float)
    return vals
...
def main():
    ...
    if ref_mean.shape != tgt_mean.shape or (not np.allclose(em_ref, em_tgt)) or (not np.allclose(ex_ref, ex_tgt)):
        # We can still proceed via interpolation sampling, but surface must be on target grid.
    ...
    if nm_points:
        # Take closest coordinate on the TARGET grid (what we will correct)
        anchors = snap_points_to_grid(nm_points, ex_tgt, em_tgt)
        anchor_mode = "coords_nm_snapped"
        print(f"Parsed {len(nm_points)} nm points from coords file; snapped to {len(anchors)} unique grid points.")
    else:
        # coords file is likely chirality-only; infer how many anchors to pick
        inferred = len(chir) if chir else 12
        n_anchors = args.auto_peaks_n if args.auto_peaks_n > 0 else inferred
        n_anchors = max(n_anchors, 8)  # ensure enough for surface stability
        ...
        # anchors are in Batch1 grid; but we sample via interpolators anyway.
        anchor_mode = "auto_peaks_from_ref_mean"
    ...
    # sample ratios (ref/tgt) at anchor points using interpolators (works even if grids differ)
    ...
    tgt_vals = np.where(np.abs(tgt_vals) < args.eps, args.eps, tgt_vals)
    ratios = ref_vals / tgt_vals
    ratios = np.where(np.isfinite(ratios), ratios, 1.0)

    # interpolate in log-space (more stable)
    log_ratios = np.log(np.maximum(ratios, args.eps))
    grid_x, grid_y = np.meshgrid(ex_tgt, em_tgt)  # x=Ex, y=Em
    surface_log = griddata(anchors, log_ratios, (grid_x, grid_y), method="linear")
    mask = np.isnan(surface_log)
    if np.any(mask):
        surface_log[mask] = griddata(anchors, log_ratios, (grid_x[mask], grid_y[mask]), method="nearest")
    surface = np.exp(surface_log)

    # QC stats
    surf_mean = float(np.mean(surface))
    surf_min = float(np.min(surface))
    surf_max = float(np.max(surface))
    surf_std = float(np.std(surface))

    print("\n--- 3) Save correction surface + debug ---")
    # Save with axes for easy inspection
    ...
    for p in it:
        ...
        save_path = (out_root / rel).with_suffix(".xlsx")
