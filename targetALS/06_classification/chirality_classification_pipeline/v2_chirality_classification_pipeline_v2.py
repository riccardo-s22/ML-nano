...
MAX_FEATURES = 20              # raised from 5; engineered features reduce redundancy
...
def run_pipeline():
    ...
    print("=" * 70)
    print("CHIRALITY DESCRIPTOR CLASSIFICATION PIPELINE — V2 (IMPROVED)")
    print("=" * 70)
    ...
    # Engineer temporal features (on full matrix — no label dependency!)
    X, feature_cols = engineer_temporal_features(X_raw, feature_cols_raw)
    n_new = len(feature_cols) - len(feature_cols_raw)
    print(f"After temporal engineering: {X.shape[1]} features (+{n_new} delta features)")
    ...
    summary_df.to_csv('v2_classification_summary.csv', index=False)
    ...
    pred_df.to_csv('v2_per_sample_predictions.csv', index=False)

    # Conserved features
    with open('v2_conserved_features.json', 'w') as f:
    ...
    with open('v2_pipeline_config.json', 'w') as f:
        json.dump(eng_summary, f, indent=2)

    print(f"\n{'='*70}")
    print("FILES SAVED")
    print(f"{'='*70}")
    print("  v2_classification_summary.csv")
    print("  v2_per_sample_predictions.csv")
    print("  v2_conserved_features.json")
    print("  v2_pipeline_config.json")
    print("\nDone!")
