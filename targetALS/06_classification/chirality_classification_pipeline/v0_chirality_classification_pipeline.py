...
def inner_loocv(X_train, y_train, clf_name, clf_factory):
    """
    Run LOOCV on training set with per-iteration feature selection.
    Returns: predictions, selected features per iteration, fold-level stable features.
    """
    n_train = len(y_train)
    loo = LeaveOneOut()
    ...
    for loo_train_idx, loo_test_idx in loo.split(X_train, y_train):
        ...
        for idx in ranked_indices:
            ...
            # Use the max single-feature power as proxy
            powers = []
            for s in selected:
...
def train_outer_model(X_train, y_train, X_test, stable_features, clf_factory):
...
for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    for clf_name, clf_factory in classifiers.items():
        ...
        # Inner LOOCV: feature selection + get stable features
        loo_preds, loo_probs, stable_feats, feat_counter = inner_loocv(
            X_train, y_train, clf_name, clf_factory
        )
        ...
        preds, probs = train_outer_model(X_train, y_train, X_test, stable_feats, clf_factory)
