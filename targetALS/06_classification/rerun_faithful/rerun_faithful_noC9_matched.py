"""
MATCHED-FOLD no-C9 variant. Isolates the effect of EXCLUDING the two C9orf72
control subjects from CV-reshuffle noise:
    NEUTK792LKM -> 10.3d__570.opj_with_emission  (CTRL)
    NEUPL452UJJ -> 30.3d__570.opj_with_emission  (CTRL)
The 5-fold split is computed on the ORIGINAL 39 subjects (identical to
rerun_faithful.py), then the two C9 controls are dropped from whichever
train/val partitions they fall in. Every OTHER subject keeps its exact fold
membership, so held-out predictions for the 37 retained subjects are directly
comparable, per-subject, to the full-cohort run. Outputs -> model_metrics_rerun_noC9_matched/.
...
OUT = os.path.join(ROOT, "model_metrics_rerun_noC9_matched")
...
def main():
    ...
    excl_mask = np.array([c in EXCLUDE_CODES for c in codes])
    for fold, (train_idx, val_idx) in enumerate(skf.split(codes, labels)):
        # MATCHED-FOLD: split computed on all 39; drop C9 controls from this fold
        drop_tr = [codes[i] for i in train_idx if excl_mask[i]]
        drop_va = [codes[i] for i in val_idx if excl_mask[i]]
        train_idx = train_idx[~excl_mask[train_idx]]
        val_idx = val_idx[~excl_mask[val_idx]]
        p(f"\n===== FOLD {fold+1}/{N_SPLITS} | train={len(train_idx)} val={len(val_idx)} =====")
        if drop_tr: p(f"  dropped from TRAIN: {drop_tr}")
        if drop_va: p(f"  dropped from VAL:   {drop_va}")
    ...
    # excluded C9 controls are never in any val fold -> stay NaN; restrict OOF to retained 37
    y_als_full = 1 - labels  # labels: ALS=0,CTRL=1 -> y_als: ALS=1
    oof_pal = 1.0 - oof_p1
    keep = ~excl_mask & ~np.isnan(oof_pal)   # retained subjects with a held-out prediction
    y_oof = y_als_full[keep]
    p_oof = oof_pal[keep]
    oof_pred_full = np.where(np.isnan(oof_pal), -1, (oof_pal >= 0.5).astype(int))
    ...
    np.savez(os.path.join(OUT, "rerun_cache.npz"),
             ...
             excluded=np.array(sorted(EXCLUDE_CODES)))
    ...
    summ = pd.concat([fold_df, pd.DataFrame([
        ...
        dict(fold="OOF", n=int(keep.sum()), accuracy=accuracy_score(y_oof, pred_oof),
             ...
             f1=f1_score(y_oof, pred_oof, zero_division=0))])], ignore_index=True)
    ...
    pd.DataFrame({"code": oof_code,
                  ...
                  "excluded_C9": excl_mask,
                  "held_out_fold": oof_fold, "P_ALS": np.round(oof_pal, 4),
                  "pred": ["EXCL" if e else ("ALS" if v == 1 else "CTRL")
                           for e, v in zip(excl_mask, oof_pred_full)],
                  "correct": [np.nan if e else (v == t)
                              for e, v, t in zip(excl_mask, oof_pred_full, y_als_full)]}).to_csv(
