...
+ CHIRALITY-INFORMED RECONSTRUCTION
...
- 12 known chirality positions guide the reconstruction loss
- Reconstruction is weighted to emphasize chirality regions
- No heights used - just (emission, excitation) coordinates
...
- Chirality-weighted reconstruction focuses on biologically relevant regions
...
# ==============================================================================
# CHIRALITY INFORMATION (from Coordinates_DNA.txt - NO HEIGHTS)
# ==============================================================================
# Each tuple is (emission_nm, excitation_nm) for known SWCNT chirality positions
CHIRALITY_ROIS = [
    (973.98, 673.94),    # [8,3]
    (987.82, 577.12),    # [6,5]
    (1047.81, 653.32),   # [7,5]
...
def create_chirality_weight_mask(emission_axis: np.ndarray, 
                                  excitation_axis: np.ndarray,
                                  roi_weight: float = 5.0,
                                  sigma_em: float = 15.0,
                                  sigma_ex: float = 8.0) -> np.ndarray:
    """
    Create a weight mask that emphasizes chirality ROI regions.
    
    Args:
        emission_axis: 1D array of emission wavelengths (nm)
        excitation_axis: 1D array of excitation wavelengths (nm)
        roi_weight: Additional weight for chirality regions (default 5.0)
        sigma_em: Gaussian spread in emission direction (nm)
        sigma_ex: Gaussian spread in excitation direction (nm)
    
    Returns:
        2D weight mask of shape (len(emission), len(excitation))
        Values >= 1.0, with higher values at chirality positions
    """
    ...
    for em_center, ex_center in CHIRALITY_ROIS:
        # Check if this chirality is within our spectral range
        if (em_center < emission_axis.min() or em_center > emission_axis.max() or
            ex_center < excitation_axis.min() or ex_center > excitation_axis.max()):
            continue
        
        # Gaussian weight centered at chirality position
        dist_em = (em_grid - em_center) ** 2 / (2 * sigma_em ** 2)
        dist_ex = (ex_grid - ex_center) ** 2 / (2 * sigma_ex ** 2)
        gaussian = np.exp(-(dist_em + dist_ex))
        
        # Add weighted Gaussian to mask
        mask += roi_weight * gaussian
...
class ExcelImageDataset(Dataset):
    ...
    - NOW ALSO EXTRACTS: emission and excitation axes for chirality mask
    """
    ...
    def __init__(self, codes: List[str], labels: List[int], 
        ...
        # Extract spectral axes from first available file (same for all)
        self.emission_axis = None
        self.excitation_axis = None
        self._extract_spectral_axes()
        
    def _extract_spectral_axes(self):
        """Extract emission and excitation wavelength axes from first file."""
        # Find first valid file
        for code in self.codes:
            filepath = os.path.join(self.timepoint_dirs[0], f"{code}.xlsx")
            if os.path.exists(filepath):
                self.emission_axis, self.excitation_axis = self._load_axes(filepath)
                return
        raise ValueError("Could not find any valid Excel file to extract axes")
    
    def _load_axes(self, filepath: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # First row contains excitation wavelengths (skip first cell which is header)
        header = rows[0]
        excitation = []
        for val in header[1:]:
            if val is not None:
                # Parse excitation value (may be like "Excitation_00520.000" or just number)
                if isinstance(val, (int, float)):
                    excitation.append(float(val))
                else:
                    s = str(val).replace('Excitation_', '').replace('nm', '').strip()
                    try:
                        excitation.append(float(s))
                    except:
                        pass
        
        # First column contains emission wavelengths (skip header row)
        emission = []
        for row in rows[1:]:
            if row[0] is not None:
                if isinstance(row[0], (int, float)):
                    ...
                    s = str(row[0]).replace('Emission_', '').replace('nm', '').strip()
                    try:
                        emission.append(float(s))
                    except:
                        pass
...
class ConvAutoencoderWithAttention(nn.Module):
    ...
    3 Latent Vectors (z1, z2, z3)
           ...
           └─→ Individual Decoders → Reconstructed Images
                                      (UNSUPERVISED, CHIRALITY-WEIGHTED)
...
class ChiralityWeightedMSELoss(nn.Module):
    """
    MSE Loss weighted by chirality importance mask.
    
    Emphasizes reconstruction quality at known chirality positions,
    guiding the encoder to focus on biologically relevant spectral regions.
    """
    
    def __init__(self, weight_mask: torch.Tensor):
        """
        Args:
            weight_mask: Tensor of shape (H, W) with weights >= 1.0
                        Higher values at chirality positions
        """
        super(ChiralityWeightedMSELoss, self).__init__()
    ...
    def forward(self, reconstructions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        """
        Compute chirality-weighted MSE loss.
        
        Args:
            reconstructions: (batch, T, C, H, W) or (batch, C, H, W)
            targets: Same shape as reconstructions
        
        Returns:
            Scalar loss value
        """
        # Compute squared error
        se = (reconstructions - targets) ** 2
        
        # Apply weight mask (broadcast to batch and channel dimensions)
        # weight_mask shape: (H, W) -> need to match (batch, T, C, H, W) or (batch, C, H, W)
        if se.dim() == 5:  # (batch, T, C, H, W)
            w = self.weight_mask.unsqueeze(0).unsqueeze(0).unsqueeze(0)  # (1, 1, 1, H, W)
        else:  # (batch, C, H, W)
            w = self.weight_mask.unsqueeze(0).unsqueeze(0)  # (1, 1, H, W)
...
def train_epoch(model, dataloader, optimizer, device, 
                chirality_loss_fn: Optional[ChiralityWeightedMSELoss] = None,
                alpha=1.0, beta=1.0):
    """
    ...
    2. Calculate dual losses (chirality-weighted reconstruction + classification)
    ...
    Args:
        ...
        device: CPU or GPU
        chirality_loss_fn: Optional chirality-weighted MSE loss (if None, uses standard MSE)
    ...
    L_total = α × L_reconstruction (chirality-weighted) + β × L_classification
    """
    ...
    if chirality_loss_fn is None:
        reconstruction_criterion = nn.MSELoss()
    else:
        reconstruction_criterion = chirality_loss_fn
    ...
    for images, labels in dataloader:
        ...
        # RECONSTRUCTION LOSS (chirality-weighted if provided)
...
def main():
    ...
    2. CHIRALITY GUIDANCE:
       - 12 known chirality (emission, excitation) positions
       - Reconstruction loss weighted to emphasize these regions
       - No height information used - just positions
    
    3. CROSS-VALIDATION STRATEGY (Critical for preventing data leakage):
    ...
    4. TRAINING APPROACH:
       - Dual-objective optimization (chirality-weighted reconstruction + classification)
    ...
    BATCH_SIZE = 4    # Small batch size appropriate for limited data
    ...
    ROI_WEIGHT = 5.0    # How much more to weight chirality regions
    SIGMA_EM = 15.0     # Gaussian spread in emission (nm)
    SIGMA_EX = 8.0      # Gaussian spread in excitation (nm)
    ...
    # ========== CREATE CHIRALITY WEIGHT MASK ==========
    # First, create a temporary dataset to get spectral axes
    temp_dataset = ExcelImageDataset(codes[:1], [0], TIMEPOINT_DIRS)
    emission_axis = temp_dataset.emission_axis
    excitation_axis = temp_dataset.excitation_axis
    
    print(f"\nSpectral axes:")
    print(f"  Emission: {emission_axis.min():.1f} - {emission_axis.max():.1f} nm ({len(emission_axis)} points)")
    print(f"  Excitation: {excitation_axis.min():.1f} - {excitation_axis.max():.1f} nm ({len(excitation_axis)} points)")
    
    # Create chirality weight mask
    chirality_mask = create_chirality_weight_mask(
    ...
    # Count how many chiralities are in range
    in_range = 0
    for em, ex in CHIRALITY_ROIS:
        if (emission_axis.min() <= em <= emission_axis.max() and
            excitation_axis.min() <= ex <= excitation_axis.max()):
            in_range += 1
    print(f"\nChirality ROIs in spectral range: {in_range}/{len(CHIRALITY_ROIS)}")
    print(f"Chirality weight mask: min={chirality_mask.min():.2f}, max={chirality_mask.max():.2f}")
    
    # Convert to tensor
    chirality_mask_tensor = torch.FloatTensor(chirality_mask).to(device)
    ...
    print(f"Chirality-weighted reconstruction: ENABLED (weight={ROI_WEIGHT})")
    print("="*70)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(codes, labels)):
        ...
        # ========== CREATE CHIRALITY LOSS FUNCTION ==========
        chirality_loss_fn = ChiralityWeightedMSELoss(chirality_mask_tensor)
        ...
        print(f"Loss weights: α={ALPHA} (chirality-weighted recon), β={BETA} (classification)")
        
        for epoch in range(NUM_EPOCHS):
            # TRAIN for one epoch
            train_loss, train_recon, train_class, train_acc = train_epoch(
                model, train_loader, optimizer, device, chirality_loss_fn, ALPHA, BETA
            )
            
            # VALIDATE on held-out subjects
            val_loss, val_recon, val_class, val_acc, attention_weights = validate(
                model, val_loader, device, chirality_loss_fn, ALPHA, BETA
            )
    ...
    print("="*70)
    print(f"""
The model has been trained with dual objectives + CHIRALITY GUIDANCE:

1. CHIRALITY-WEIGHTED RECONSTRUCTION (unsupervised):
   - Reconstruction loss emphasizes {in_range} known chirality positions
   - Weight boost at chirality ROIs: {ROI_WEIGHT}x
   - Forces encoder to learn chirality-relevant features
   - Low reconstruction loss = encoder preserves chirality information
...
CHIRALITY ROIs USED (emission, excitation in nm):
""")
    for i, (em, ex) in enumerate(CHIRALITY_ROIS):
        in_range_str = "✓" if (emission_axis.min() <= em <= emission_axis.max() and
                               excitation_axis.min() <= ex <= excitation_axis.max()) else "✗"
        print(f"  {i+1:2d}. ({em:.1f}, {ex:.1f}) {in_range_str}")
