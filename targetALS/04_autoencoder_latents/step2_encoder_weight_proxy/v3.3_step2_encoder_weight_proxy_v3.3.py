...
def strategy_analytical_z(
    ...
    The attention mechanism is non-linear and operates over ALL dims jointly,
    so we must compute the full z_agg, not just dim d in isolation.

    Sign-alignment: since Z_target may have been sign-aligned differently
    from this specific fold's output, we check correlation and flip if needed.

    Returns: (proxy [N,], info_dict)
    """
    # Full analytical z_agg for this fold
    z_agg, attn_weights = compute_z_agg_analytical(conv_feats, W_full, b_full, attn_params)

    # Extract dim d
    z_fold_d = z_agg[:, dim_d]  # [N,]

    # Sign-align to z_target
    ...
    # Scale to match z_target (affine transform: z_proxy = a*z_fold + b)
    # This is NOT fitting — it's just ensuring comparable scale/offset,
    # which is needed because each fold's z values have arbitrary scale.
    if np.std(z_fold_d) > 1e-12:
        z_proxy = (z_fold_d - z_fold_d.mean()) / z_fold_d.std() * z_target.std() + z_target.mean()
    else:
        z_proxy = z_fold_d

    r, r2, mse = _oof_stats(z_proxy, z_target)

    # Also compute per-TP contributions for diagnostics
    N, T, F = conv_feats.shape
    z_per_tp_d = np.zeros((N, T))
    for t in range(T):
