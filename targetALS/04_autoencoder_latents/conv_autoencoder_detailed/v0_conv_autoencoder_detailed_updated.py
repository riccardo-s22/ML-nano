...
def train_epoch(model, dataloader, optimizer, device, alpha=1.0, beta=1.0):
    ...
    for images, labels in dataloader:
        # Move data to device
        images = images.to(device)   # (batch, T, 1, H, W)
        ...
        _, predicted = torch.max(class_logits, dim=1)
        total += labels.size(0)
...
def validate(model, dataloader, device, alpha=1.0, beta=1.0):
    """
    Validate the model (same objectives as training, but no gradient updates).

    Returns:
        avg_loss, avg_recon_loss, avg_class_loss, accuracy, attention_weights_array
    """
    model.eval()
    ...
    if len(all_attention_weights) > 0:
        ...
        attn_array = np.zeros((0,))

    return avg_loss, avg_recon_loss, avg_class_loss, accuracy, attn_array
