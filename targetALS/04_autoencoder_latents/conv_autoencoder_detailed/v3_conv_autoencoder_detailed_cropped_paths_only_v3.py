...
def main():
    ...
    """
    # ========== CONFIGURATION ==========
    # You can now provide the cropped root folder at runtime, e.g.:
    #   python conv_autoencoder_detailed_cropped_paths_only_v3.py --tp_root "C:\\...\\cropped_em920_ex520-815"
    parser = argparse.ArgumentParser(add_help=True)
    parser.add_argument(
        "--tp_root",
        type=str,
        default=None,
        help="Root folder containing out_0h/out_6h/out_24h subfolders with .xlsx files.",
    )
    parser.add_argument(
        "--labels_csv",
        type=str,
        default="sample_labels.csv",
        help="Path to sample_labels.csv (default: sample_labels.csv in current working directory).",
    )
    args, _unknown = parser.parse_known_args()

    LABELS_CSV = args.labels_csv

    if args.tp_root is None:
        ...
        # Construct timepoint directories from the provided root
        TIMEPOINT_DIRS = [
            os.path.join(args.tp_root, "out_0h"),
            os.path.join(args.tp_root, "out_6h"),
            os.path.join(args.tp_root, "out_24h"),
        ]
