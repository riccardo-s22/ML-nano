#!/usr/bin/env python3
"""
STEP 3 v2 (TARGETED): Pixel Attribution & ROI Extraction
=========================================================
...
def load_target_features_from_config(config_path: str) -> Dict[str, List[int]]:
    """
    Load model_dims mapping from Step OPT v2 config JSON.
    Returns: {'best_model_fold1': [103, 179], 'best_model_fold3': [477, 376, 370]}
    """
    with open(config_path, 'r') as f:
        config = json.load(f)

    model_dims = config.get("model_dims", {})
    if not model_dims:
        raise ValueError(f"No model_dims found in config: {config_path}")

    print(f"Loaded target features from config: {config_path}")
    for mname, dims in model_dims.items():
        print(f"  {mname}: dims {dims}")
    return model_dims


def parse_target_features_from_list(feature_list: List[str]) -> Dict[str, List[int]]:
    """
    Fallback: parse feature names like 'best_model_fold1_dim103'
    Returns: {'best_model_fold1': [103], ...}
    """
    needed = {}
    ...
    for f in tp_path.glob("*.xlsx"):
        ...
        for suffix in ["_0h", "_6h", "_24h"]:
            if stem.endswith(suffix):
                if stem[:-len(suffix)] == code:
                    return str(f)
                if stem[:-len(suffix)].replace('.', '_') == code:
                    return str(f)
    return None
...
def main():
    parser = argparse.ArgumentParser(description="Step 3 v2: Targeted ROI Extraction (reads Step OPT config)")
    ...
    parser.add_argument("--output_dir", type=str, default="./step3_targeted_rois_v2")
    ...
    print("=" * 70)
    print("STEP 3 v2: TARGETED PIXEL ATTRIBUTION + MSE FILTER")
    print("=" * 70)
    ...
    if args.opt_config and Path(args.opt_config).exists():
        TARGET_FEATURES = load_target_features_from_config(args.opt_config)
        # Also copy the config to output for traceability
        import shutil
        shutil.copy2(args.opt_config, outdir / "source_config.json")
    elif args.target_features:
        feat_list = [f.strip() for f in args.target_features.split(",")]
        TARGET_FEATURES = parse_target_features_from_list(feat_list)
        print(f"Using manual target features: {TARGET_FEATURES}")
    else:
    ...
    for m_name, target_dims in TARGET_FEATURES.items():
        ...
        for d in target_dims:
            for t_idx, tp_name in enumerate(TP_NAMES):
                ...
                n_px = mask.sum()
                total_px = H * W
                print(f"      Dim {d} - {tp_name}: Valid Pixels = {n_px}/{total_px} "
                      f"({100*n_px/total_px:.1f}%)")
                ...
                roi_summary_rows.append({
                    ...
                    "roi_fraction": float(n_px / total_px),
                    "imp_map_mean": float(imp_map.mean()),
                    "imp_map_max": float(imp_map.max()),
                    "mse_map_mean": float(mse_map.mean()),
                })
    ...
    roi_summary_df.to_csv(outdir / "roi_summary.csv", index=False)
    print(f"Saved ROI summary to roi_summary.csv")

    # Save the config used (for Step 4 to consume)
    step3_config = {
        "step3_version": "v2",
        "mask_percentile": args.mask_percentile,
        "mse_threshold": args.mse_threshold,
        "target_features": {m: [int(d) for d in dims] for m, dims in TARGET_FEATURES.items()},
