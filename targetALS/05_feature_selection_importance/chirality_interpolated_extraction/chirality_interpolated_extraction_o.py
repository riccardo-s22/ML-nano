...
Output columns per chirality per timepoint:
  {chir}_gauss_max_{tp}         – fitted peak intensity (amplitude + baseline)
  {chir}_gauss_em_center_{tp}   – emission center of fitted peak (nm)
  {chir}_gauss_ex_center_{tp}   – excitation at local max (nm)  
  {chir}_gauss_fwhm_{tp}        – full width at half maximum (nm)
  {chir}_gauss_auc_{tp}         – area under the peak curve
  {chir}_gauss_sigma_left_{tp}  – sigma on the short-wavelength side (nm)
  {chir}_gauss_sigma_right_{tp} – sigma on the long-wavelength side (nm)
  ...
  {chir}_gauss_r2_{tp}          – R² of the split-Gaussian fit
  {chir}_gauss_rmse_{tp}        – RMSE of the fit (original intensity units)
  {chir}_gauss_npts_{tp}        – number of emission points used for fitting
...
def extract_consistent_gradient_points(
    ...
    3.  The gradient walk gives the maximal extent of monotone descent.
        ...
        capped at `window_factor` × half-width on each side, where the
    ...
    # ------------------------------------------------------------------
    # Step 3: cap the window at window_factor × half-width
    # ------------------------------------------------------------------
    # Local baseline from the gradient-walk endpoints
    ...
    if hw_right is None and hw_left is not None:
        ...
        # Very flat peak – use a default window
        hw_right = hw_left = 15.0  # nm

    # Cap: gradient extent vs window_factor × half-width
    max_right_nm = window_factor * hw_right
    max_left_nm = window_factor * hw_left

    # Find the actual right boundary (min of gradient stop and cap)
    ...
    if len(final_indices) < 6:
        half_win = 20  # points on the fine grid
...
def fit_gaussian_and_extract(
    ...
    with very small fluorescence intensities (~10⁻¹⁴).  The split Gaussian
    ...
    --------------------
    gauss_max         – total peak intensity (fitted amp + baseline), original scale
    gauss_em_center   – emission center of fitted peak (nm)
    gauss_fwhm        – full width at half maximum ≈ 1.177·(σ_left + σ_right) (nm)
    gauss_auc         – area under the peak (amplitude × √(2π) × (σL + σR)/2)
    gauss_sigma_left  – sigma on the short-wavelength side (nm)
    gauss_sigma_right – sigma on the long-wavelength side (nm)
    gauss_skew        – intensity-weighted skewness of the data points
    gauss_kurt        – intensity-weighted excess kurtosis of the data points
    gauss_r2          – R² of the split-Gaussian fit
    gauss_rmse        – RMSE of the fit in original intensity units
    gauss_npts        – number of emission points used for fitting
    ...
    try:
        ...
        # FWHM: half-width at half-max on each side is 1.177 × sigma
        fwhm = 1.17741 * (sig_l + sig_r)  # √(2·ln2) ≈ 1.17741
        ...
        # Goodness-of-fit (on normalised data → R² is scale-invariant)
