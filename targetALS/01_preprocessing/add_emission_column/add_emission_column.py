...
# Usage examples:
#   python add_emission_column.py /path/to/file.xlsx
#   python add_emission_column.py /path/to/folder_with_files
#   python add_emission_column.py /data --emission-file emission_values.txt --outdir /data/out
#
# Notes:
# - The script reads emission values (one per line) from --emission-file (defaults to 'emission_values.txt').
# - For Excel files, ALL sheets are processed.
# - Output files are written with the suffix '_with_emission' unless --inplace is used.
# - If the number of emission values != number of data rows, the script will trim or pad with blanks (NaN).
#
# Requires: pandas, openpyxl (for Excel), numpy
# Install (if needed): pip install pandas openpyxl numpy
...
def read_emission_values(path: Path):
    vals = []
...
def insert_emission_column(df: pd.DataFrame, emission_vals, col_name='Emission'):
    n = len(df)
    # match length
...
def process_csv(path: Path, emission_vals, inplace=False, outdir: Path | None = None):
...
def process_excel(path: Path, emission_vals, inplace=False, outdir: Path | None = None):
